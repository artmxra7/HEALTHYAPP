...
class Finance extends MX_Controller {
    ...
    public function addPayment() {
        ...
        $medicine_selected = $this->input->post('medicine_id');
        $quantity_item = $this->input->post('quantity_item');
        $quantity_medicine = $this->input->post('quantity_medicine');
        ...
        if (empty($item_selected)) {
            ...
            $medicine_quantity_array = array();
            $item_quantity_array = array_combine($item_selected, $quantity_item);
            if ($medicine_selected) {
                $medicine_quantity_array = array_combine($medicine_selected, $quantity_medicine);
            }
        }
        $cat_and_price = array();
        if (!empty($item_quantity_array)) {
            ...
            $this->session->set_flashdata('feedback', 'Atend Item The Required Fields');
            redirect('finance/addPaymentView');
        }

        $medicine_name = '';
        $amount_by_medicine = [];
        if (!empty($medicine_quantity_array)) {
            foreach ($medicine_quantity_array as $key => $value) {
                $current_item2 = $this->finance_model->getPaymentMedicineById($key);
                $id_med = $current_item2->id;
                $category_price2 = $current_item2->s_price;
                $category_type2 = $current_item2->category;
                $qty = $value;
                $med_and_price[] = $id_med . '*' . $category_price2 . '*' . $category_type2 . '*' . $qty;
                $amount_by_medicine[] = $category_price2 * $qty;
            }
            $medicine_name = implode(',', $med_and_price);
        }
...
// Validating Price Field
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (!empty($p_name)) {
            	...
            	foreach ($med_and_price as $value) {
            	    $med_and_price_exp = explode('*', $value);
            	    $medicine = $this->db->get_where('medicine', ['id' => $med_and_price_exp[0]])->row();
            	    $this->db->where('id', $med_and_price_exp[0])
            	        ->update('medicine', ['quantity' => $medicine->quantity - end($med_and_price_exp)]);   
            	}
...
// Adding New Patient
            ...
            $amount = array_sum($amount_by_category) + array_sum($amount_by_medicine);
            $sub_total = $amount;
	...
	$data['credits'] = $this->db->where('amount_received < gross_total')
            ->where('status', 'unpaid')
            ->get('payment')->result();
    ...
    public function setPaid($id = null)
    {
        if ($id) {
            $deposited_amount = $this->db->select_sum('deposited_amount', 'total')
                ...
                ->where('payment_id', $id)
                ->get()->row_array();
            $payment =  $this->db->where('id', $id)->get('payment')->row_array();
            $this->db->where('id', $id)->update('payment', [
                'status'          => 'paid',
                'amount_received' => $payment['gross_total'],
            ]);
            $this->db->insert('patient_deposit', [
                'patient'            => $payment['patient'],
                'payment_id'         => $id,
                'date'               => time(),
                'deposited_amount'   => $payment['gross_total'] - $deposited_amount['total'],
                'amount_received_id' => $id . '.gp',
                'deposit_type'       => 'Cash',
                'user'               => $this->ion_auth->get_user_id(),
                'hospital_id'        => $payment['hospital_id'],
            ]);
            $this->session->set_flashdata('feedback', 'Paided');
            redirect('finance/creditReport');
        } else {
            redirect('finance/creditReport');
        }
    }
