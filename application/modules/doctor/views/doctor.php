            ...
            <div class="modal-body row">
                <form role="form" action="doctor/addNew" class="clearfix" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <select class="form-control m-bot15 js-example-basic-single" name="jenis_kel" value=''>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form role="form" id="editDoctorForm" class="clearfix" action="doctor/addNew" method="post" enctype="multipart/form-data">
                   ...
                   <div class="form-group col-md-6">
                        <label for="exampleInputEmail1">Gender</label>
                        <input type="text" class="form-control" name="jenis_kel" id="exampleInputEmail1" value='' placeholder="">
                    </div>
    ...
    $(document).ready(function () {
        $(".table").on("click", ".editbutton", function () {
            ...
            $.ajax({
                ...
                $('#editDoctorForm').find('[name="nik"]').val(response.doctor.nik).end()
                $('#editDoctorForm').find('[name="name"]').val(response.doctor.name).end()
                $('#editDoctorForm').find('[name="jenis_kel"]').val(response.doctor.jenis_kel).end()
                ...
                $('#editDoctorForm').find('[name="salary"]').val(response.doctor.salary).end()
    ...
    $(document).ready(function () {
        $(".table").on("click", ".inffo", function () {
            ...
            $.ajax({
                ...
                // Populate the form fields with the data returned from server
                $('#editDoctorForm').find('[name="id"]').val(response.doctor.id).end()
                $('.nikClass').append(response.doctor.nik).end()
                $('.nameClass').append(response.doctor.name).end()
                $('.jenis_kelClass').append(response.doctor.jenis_kel).end()
                ...
                $('.profileClass').append(response.doctor.profile).end()
                 $('.salaryClass').append(response.doctor.salary).end()
                if (typeof response.doctor.img_url !== 'undefined' && response.doctor.img_url != '') {
